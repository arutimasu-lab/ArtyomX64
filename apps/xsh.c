...
static int tty_read(char *buf, unsigned long count)
{
    int ret;
    __asm__ volatile("int $0x80"
        : "=a"(ret)
        : "a"(165), "D"((long)buf), "S"((long)count)
        : "memory");
    return ret;
}
...
static void append_tty_char(char ch)
{
    ...
    if (ch == '\b' || ch == 127) {
        int len = 0;
        while (lines[line_count - 1][len]) len++;
        if (len > 0) lines[line_count - 1][len - 1] = 0;
        return;
    }

    int len = 0;
    while (lines[line_count - 1][len]) len++;
    if (len >= LINE_LEN - 1) { push_line(""); len = 0; }
    lines[line_count - 1][len] = ch;
...
static bool run_cmd(char *raw)
{
    ...
    else if (xstrcmp(raw, "exec") == 0) { if (*arg) { exec(arg); pty_mode = true; } else push_line("usage: exec <file>"); }
...
void _start(void)
{
    ...
    while (alive) {
        ...
        while (ax_poll(canvas, &ev) > 0) {
            ...
            if (ev.type != AX_EV_KEY || pty_mode) continue;
            char k = (char)ev.key;
            if (k == '\n' || k == '\r') {
                input[input_len] = 0;
                push_prefixed("$ ", input);
        ...
        char ttybuf[TTY_BUF];
        int n = tty_read(ttybuf, sizeof(ttybuf));
        if (n > 0) {
            for (int i = 0; i < n; i++) append_tty_char(ttybuf[i]);
            dirty = true;
        }
